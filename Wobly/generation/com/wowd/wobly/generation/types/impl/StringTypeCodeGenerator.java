...
public class StringTypeCodeGenerator implements TypeCodeGenerator
{
	@Override
			...
			Format special, WoblyField of, int depth)
	{
		...
		{
			boolean vSize = special == Format.BYTES_SIZE_COMPRESSED || special == Format.DEFAULT;
			result.println(BUFFERS + ".putStringUTF8(buf, " + obj + ", " + vSize + ");");
		}
			...
			Format special, WoblyField of, int depth)
	{
		...
		{
			boolean vSize = special == Format.BYTES_SIZE_COMPRESSED || special == Format.DEFAULT;
			result.println(obj + " = " + BUFFERS + ".getStringUTF8(buf, " + vSize + ");");
		}
		else throw new IllegalArgumentException();
		return null;
	}
			...
			Format special, WoblyField of, int depth)
	{
		...
		{
			boolean vSize = special == Format.BYTES_SIZE_COMPRESSED || special == Format.DEFAULT;
			result.println("size += " + BUFFERS + ".sizeStringUTF8(" + obj + ", " + vSize + ");");
		}
	}
