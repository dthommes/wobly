...
import static com.wowd.wobly.generation.WoblyGeneratorUtils.getReadStaticName;
...
public class WoblyTypeCodeGenerator implements TypeCodeGenerator
{
	@Override
	...
	{
		if (Wobly.class.isAssignableFrom(clazz)) {
			result.println(obj + ".write(buf);");
		}
			...
			Format special, WoblyField of, int depth)
	{
		...
		if (Wobly.class.isAssignableFrom(clazz)) {
			result.println(obj + " = " + typeName + "." + getReadStaticName(clazz) + "(buf);");
		}
		else throw new IllegalArgumentException();
		return null;
	}
	...
	{
		if (Wobly.class.isAssignableFrom(clazz)) {
			result.append("size += " + obj + ".getSize();\n");
		}
		else throw new IllegalArgumentException();
	}
	...
	{
		return Wobly.class.isAssignableFrom(clazz);
	}
